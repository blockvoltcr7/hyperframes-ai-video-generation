...
export function App() {
  ...
  return <div className="shell">
    ...
    <main className="main">
      ...
      {loading ? <div className="loading-line">Waking the local runner…</div> : view === "projects" ? <Projects projects={projects} jobs={jobs} selected={selected} onSelect={setSelected} onCheck={async (slug) => { const job = await api.check(slug); setJobs((items) => [job, ...items]); setNotice(`Validation started for ${slug}`); }} onPreview={async (slug) => { const job = await api.preview(slug); setJobs((items) => [job, ...items]); setNotice(`Preview server started for ${slug}`); }} onRender={async (slug) => { const job = await api.render(slug); setJobs((items) => [job, ...items]); setNotice(`Render started for ${slug}. Rendering is explicit and will not overwrite an existing output implicitly.`); }} onCreate={() => setView("create")} /> : view === "create" ? <Create onComplete={(job) => { setJobs((items) => [job, ...items]); setView("projects"); setNotice("Codex generation started. Watch its progress in Recent activity."); }} /> : <Health checks={checks} onRun={() => void loadHealth()} />}
    </main>
  </div>;
}

function Projects({ projects, jobs, selected, onSelect, onCheck, onPreview, onRender, onCreate }: { projects: ProjectSummary[]; jobs: JobSummary[]; selected: ProjectSummary | null; onSelect: (project: ProjectSummary) => void; onCheck: (slug: string) => void; onPreview: (slug: string) => void; onRender: (slug: string) => void; onCreate: () => void }) {
  ...
  return <>
    ...
    <section className="project-layout"><div className="project-list">{projects.length === 0 ? <Empty onCreate={onCreate} /> : projects.map((project) => <ProjectRow key={project.slug} project={project} selected={selected?.slug === project.slug} onClick={() => onSelect(project)} />)}</div><ProjectDetail project={selected ?? projects[0] ?? null} onCheck={onCheck} onPreview={onPreview} onRender={onRender} /></section>
    <section className="section-heading activity-heading"><div><span className="eyebrow">OPERATIONS</span><h2>Recent activity</h2></div></section><div className="activity-list">{recentJobs.length ? recentJobs.map((job) => <Activity key={job.id} job={job} />) : <div className="empty-activity">No jobs yet. Your next Codex generation will appear here.</div>}</div>
  </>;
}
...
function ProjectDetail({ project, onCheck, onPreview, onRender }: { project: ProjectSummary | null; onCheck: (slug: string) => void; onPreview: (slug: string) => void; onRender: (slug: string) => void }) { if (!project) return <div className="detail-panel placeholder"><span className="detail-number">01</span><h3>Select a project</h3><p>Your project details, artifacts, and validation actions will live here.</p></div>; const existing = project.artifacts.filter((item) => item.exists).length; return <div className="detail-panel"><div className="detail-top"><div><span className="eyebrow">SELECTED PROJECT</span><h3>{project.title}</h3><p className="path">{project.path}</p></div><span className={`status ${project.status}`}><i />{project.status}</span></div><div className="detail-preview"><div className="preview-grid" /><div className="preview-label">{templateMeta[project.template as keyof typeof templateMeta]?.label ?? "HyperFrames"}</div><div className="preview-play">▶</div></div><div className="artifact-head"><span>Artifacts</span><span>{existing}/{project.artifacts.length} present</span></div><div className="artifact-list">{project.artifacts.map((item) => <div className="artifact" key={item.relativePath}><span className={item.exists ? "artifact-check" : "artifact-missing"}>{item.exists ? "✓" : "–"}</span><span>{item.relativePath}</span><span>{item.exists ? formatBytes(item.sizeBytes) : "missing"}</span></div>)}</div><div className="detail-actions"><button className="secondary" onClick={() => onCheck(project.slug)}>Check <span>↗</span></button><button className="secondary" onClick={() => onPreview(project.slug)}>Preview <span>↗</span></button><button className="primary" onClick={() => onRender(project.slug)}>Render <span>↗</span></button></div></div>; }
function Create({ onComplete }: { onComplete: (job: JobSummary) => void }) { const [topic, setTopic] = useState(""); const [workflow, setWorkflow] = useState("adaptive"); const [template, setTemplate] = useState("classic"); const [images, setImages] = useState("auto"); const [busy, setBusy] = useState(false); const [error, setError] = useState<string | null>(null); async function submit(event: FormEvent) { event.preventDefault(); setBusy(true); setError(null); try { onComplete(await api.startGeneration({ topic, workflow, template, images })); } catch (err) { setError(err instanceof Error ? err.message : "Could not start generation"); } finally { setBusy(false); } } return <section className="create-grid"><div className="create-intro"><span className="eyebrow">CODEX GENERATION</span><h2>Give the next idea a frame.</h2><p>Codex will use project-scoped HyperFrames skills to research, storyboard, narrate, caption, compose, and visually verify a previewable video.</p><div className="flow"><span>01 Input</span><b>→</b><span>02 Direct</span><b>→</b><span>03 Preview</span></div></div><form className="create-form" onSubmit={submit}><label>What should the short explain?<textarea value={topic} onChange={(event) => setTopic(event.target.value)} placeholder="e.g. Why vector databases make semantic search possible" required rows={4} /></label><label>Generation workflow<select value={workflow} onChange={(event) => setWorkflow(event.target.value)}><option value="adaptive">Adaptive explainer — variable scenes + captions</option><option value="template">Template short — legacy four-phase system</option></select></label>{workflow === "template" && <label>Visual system<select value={template} onChange={(event) => setTemplate(event.target.value)}><option value="classic">Classic — blue / editorial</option><option value="archon">Archon — cyan / magenta</option><option value="anthropic">Anthropic — orange / cream</option></select></label>}<label>Original image assets<select value={images} onChange={(event) => setImages(event.target.value)}><option value="auto">Auto — generate only when story-justified</option><option value="off">Off — native and sourced visuals only</option><option value="required">Required — generation must include an original asset</option></select></label><div className="form-note"><span>⌁</span><p>Adaptive mode uses the current HyperFrames storyboard, caption, registry, media, and snapshot workflows. Rendering remains a separate explicit action.</p></div>{error && <div className="form-error">{error}</div>}<button className="primary full" disabled={busy}>{busy ? "Starting Codex…" : "Start Codex generation →"}</button></form></section>; }
